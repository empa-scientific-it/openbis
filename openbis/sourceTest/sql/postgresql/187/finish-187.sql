...
ALTER TABLE ONLY public.events_search
...
CREATE INDEX data_set_properties_search_index ON public.data_set_properties USING gin (tsvector_document);
CREATE UNIQUE INDEX data_set_properties_unique_cvte ON public.data_set_properties USING btree (dstpt_id, cvte_id) WHERE (is_unique AND (cvte_id IS NOT NULL));
CREATE UNIQUE INDEX data_set_properties_unique_mate ON public.data_set_properties USING btree (dstpt_id, mate_prop_id) WHERE (is_unique AND (mate_prop_id IS NOT NULL));
CREATE UNIQUE INDEX data_set_properties_unique_samp ON public.data_set_properties USING btree (dstpt_id, samp_prop_id) WHERE (is_unique AND (samp_prop_id IS NOT NULL));
CREATE UNIQUE INDEX data_set_properties_unique_value ON public.data_set_properties USING btree (dstpt_id, value) WHERE (is_unique AND (value IS NOT NULL));
...
CREATE INDEX dspr_sapr_fk_i ON public.data_set_properties USING btree (samp_prop_id);
...
CREATE INDEX events_search_entity_project_i ON public.events_search USING btree (entity_project);
CREATE INDEX events_search_entity_project_perm_id_i ON public.events_search USING btree (entity_project_perm_id);
CREATE INDEX events_search_entity_registerer_i ON public.events_search USING btree (entity_registerer);
CREATE INDEX events_search_entity_registration_timestamp_i ON public.events_search USING btree (entity_registration_timestamp);
CREATE INDEX events_search_entity_space_i ON public.events_search USING btree (entity_space);
CREATE INDEX events_search_entity_space_perm_id_i ON public.events_search USING btree (entity_space_perm_id);
CREATE INDEX events_search_exac_id_i ON public.events_search USING btree (exac_id);
CREATE INDEX events_search_pers_id_registerer_i ON public.events_search USING btree (pers_id_registerer);
CREATE INDEX events_search_registration_timestamp_i ON public.events_search USING btree (registration_timestamp);
...
CREATE INDEX experiment_properties_search_index ON public.experiment_properties USING gin (tsvector_document);
CREATE UNIQUE INDEX experiment_properties_unique_cvte ON public.experiment_properties USING btree (etpt_id, cvte_id) WHERE (is_unique AND (cvte_id IS NOT NULL));
CREATE UNIQUE INDEX experiment_properties_unique_mate ON public.experiment_properties USING btree (etpt_id, mate_prop_id) WHERE (is_unique AND (mate_prop_id IS NOT NULL));
CREATE UNIQUE INDEX experiment_properties_unique_samp ON public.experiment_properties USING btree (etpt_id, samp_prop_id) WHERE (is_unique AND (samp_prop_id IS NOT NULL));
CREATE UNIQUE INDEX experiment_properties_unique_value ON public.experiment_properties USING btree (etpt_id, value) WHERE (is_unique AND (value IS NOT NULL));
...
CREATE INDEX expr_sapr_fk_i ON public.experiment_properties USING btree (samp_prop_id);
...
CREATE INDEX material_properties_search_index ON public.material_properties USING gin (tsvector_document);
CREATE UNIQUE INDEX material_properties_unique_cvte ON public.material_properties USING btree (mtpt_id, cvte_id) WHERE (is_unique AND (cvte_id IS NOT NULL));
CREATE UNIQUE INDEX material_properties_unique_mate ON public.material_properties USING btree (mtpt_id, mate_prop_id) WHERE (is_unique AND (mate_prop_id IS NOT NULL));
CREATE UNIQUE INDEX material_properties_unique_value ON public.material_properties USING btree (mtpt_id, value) WHERE (is_unique AND (value IS NOT NULL));
...
CREATE INDEX prrelh_main_proj_fk_samp_fk_i ON public.project_relationships_history USING btree (main_proj_id, samp_id);
...
CREATE INDEX samp_identifier_i ON public.samples_all USING btree (sample_identifier);
...
CREATE INDEX sample_properties_search_index ON public.sample_properties USING gin (tsvector_document);
CREATE UNIQUE INDEX sample_properties_unique_cvte ON public.sample_properties USING btree (stpt_id, cvte_id) WHERE (is_unique AND (cvte_id IS NOT NULL));
CREATE UNIQUE INDEX sample_properties_unique_mate ON public.sample_properties USING btree (stpt_id, mate_prop_id) WHERE (is_unique AND (mate_prop_id IS NOT NULL));
CREATE UNIQUE INDEX sample_properties_unique_samp ON public.sample_properties USING btree (stpt_id, samp_prop_id) WHERE (is_unique AND (samp_prop_id IS NOT NULL));
CREATE UNIQUE INDEX sample_properties_unique_value ON public.sample_properties USING btree (stpt_id, value) WHERE (is_unique AND (value IS NOT NULL));
...
CREATE INDEX sapr_sapr_fk_i ON public.sample_properties USING btree (samp_prop_id);
...
CREATE RULE content_copies_history_delete AS
    ON DELETE TO public.content_copies DO  UPDATE public.data_set_copies_history SET valid_until_timestamp = now()
...
CREATE RULE data_relationship_insert AS
   ...
   WHERE (new.del_id IS NULL) DO ( INSERT INTO public.data_set_relationships_history (id, main_data_id, relation_type, data_id, entity_kind, entity_perm_id, pers_id_author, valid_from_timestamp, ordinal)
 ...
 INSERT INTO public.data_set_relationships_history (id, main_data_id, relation_type, data_id, entity_kind, entity_perm_id, pers_id_author, valid_from_timestamp, ordinal)
...
CREATE RULE data_relationship_trash_revert_update AS
   ...
   WHERE ((old.del_id IS NOT NULL) AND (new.del_id IS NULL)) DO ( INSERT INTO public.data_set_relationships_history (id, main_data_id, relation_type, data_id, entity_kind, entity_perm_id, pers_id_author, valid_from_timestamp, ordinal)
  VALUES (nextval('public.data_set_relationships_history_id_seq'::regclass), new.data_id_parent, ( SELECT upper((relationship_types.parent_label)::text) AS upper
           FROM public.relationship_types
          WHERE ((relationship_types.id)::bigint = (new.relationship_id)::bigint)), new.data_id_child, 'DATA SET'::text, ( SELECT data_all.code
           FROM public.data_all
...
CREATE RULE data_set_properties_delete AS
   ...
   WHERE ((((old.value IS NOT NULL) AND (decode(replace("substring"((old.value)::text, 1, 1), '\'::text, '\\'::text), 'escape'::text) <> '\xefbfbd'::bytea)) OR (old.cvte_id IS NOT NULL) OR (old.mate_prop_id IS NOT NULL) OR (old.samp_prop_id IS NOT NULL)) AND (( SELECT data_all.del_id
           FROM public.data_all
          WHERE ((data_all.id)::bigint = (old.ds_id)::bigint)) IS NULL)) DO  INSERT INTO public.data_set_properties_history (id, ds_id, dstpt_id, value, vocabulary_term, material, sample, pers_id_author, valid_from_timestamp, valid_until_timestamp)
...
CREATE RULE data_set_properties_update AS
   ...
   WHERE (((old.value IS NOT NULL) AND (decode(replace("substring"((old.value)::text, 1, 1), '\'::text, '\\'::text), 'escape'::text) <> '\xefbfbd'::bytea) AND ((old.value)::text <> (new.value)::text)) OR ((old.cvte_id IS NOT NULL) AND ((old.cvte_id)::bigint <> (new.cvte_id)::bigint)) OR ((old.mate_prop_id IS NOT NULL) AND ((old.mate_prop_id)::bigint <> (new.mate_prop_id)::bigint)) OR ((old.samp_prop_id IS NOT NULL) AND ((old.samp_prop_id)::bigint <> (new.samp_prop_id)::bigint))) DO  INSERT INTO public.data_set_properties_history (id, ds_id, dstpt_id, value, vocabulary_term, material, sample, pers_id_author, valid_from_timestamp, valid_until_timestamp)
  VALUES (nextval('public.data_set_property_id_seq'::regclass), old.ds_id, old.dstpt_id, old.value, ( SELECT ((((t.code)::text || ' ['::text) || (v.code)::text) || ']'::text)
           FROM (public.controlled_vocabulary_terms t
             JOIN public.controlled_vocabularies v ON (((t.covo_id)::bigint = (v.id)::bigint)))
          WHERE ((t.id)::bigint = (old.cvte_id)::bigint)), ( SELECT ((((m.code)::text || ' ['::text) || (mt.code)::text) || ']'::text)
...
CREATE RULE dataset_experiment_insert AS
   ...
   WHERE ((new.expe_id IS NOT NULL) AND (new.samp_id IS NULL)) DO ( INSERT INTO public.experiment_relationships_history (id, main_expe_id, relation_type, data_id, entity_kind, entity_perm_id, pers_id_author, valid_from_timestamp)
  VALUES (nextval('public.experiment_relationships_history_id_seq'::regclass), new.expe_id, 'OWNER'::text, new.id, 'DATA SET'::text, new.code, new.pers_id_modifier, new.modification_timestamp);
 INSERT INTO public.data_set_relationships_history (id, main_data_id, relation_type, expe_id, entity_kind, entity_perm_id, pers_id_author, valid_from_timestamp)
  VALUES (nextval('public.data_set_relationships_history_id_seq'::regclass), new.id, 'OWNED'::text, new.expe_id, 'EXPERIMENT'::text, ( SELECT experiments_all.perm_id
...
CREATE RULE dataset_experiment_update AS
  ...
  WHERE (((experiment_relationships_history.main_expe_id)::bigint = (old.expe_id)::bigint) AND ((experiment_relationships_history.data_id)::bigint = (old.id)::bigint) AND (experiment_relationships_history.valid_until_timestamp IS NULL));
 INSERT INTO public.experiment_relationships_history (id, main_expe_id, relation_type, data_id, entity_kind, entity_perm_id, pers_id_author, valid_from_timestamp)
  VALUES (nextval('public.experiment_relationships_history_id_seq'::regclass), new.expe_id, 'OWNER'::text, new.id, 'DATA SET'::text, new.code, new.pers_id_modifier, new.modification_timestamp);
 UPDATE public.data_set_relationships_history SET valid_until_timestamp = new.modification_timestamp
  WHERE (((data_set_relationships_history.main_data_id)::bigint = (old.id)::bigint) AND ((data_set_relationships_history.expe_id)::bigint = (old.expe_id)::bigint) AND (data_set_relationships_history.valid_until_timestamp IS NULL));
 INSERT INTO public.data_set_relationships_history (id, main_data_id, relation_type, expe_id, entity_kind, entity_perm_id, pers_id_author, valid_from_timestamp)
...
CREATE RULE dataset_sample_insert AS
    ON INSERT TO public.data_all
   WHERE (new.samp_id IS NOT NULL) DO ( INSERT INTO public.sample_relationships_history (id, main_samp_id, relation_type, data_id, entity_kind, entity_perm_id, pers_id_author, valid_from_timestamp)
  VALUES (nextval('public.sample_relationships_history_id_seq'::regclass), new.samp_id, 'OWNER'::text, new.id, 'DATA SET'::text, new.code, new.pers_id_modifier, new.modification_timestamp);
 INSERT INTO public.data_set_relationships_history (id, main_data_id, relation_type, samp_id, entity_kind, entity_perm_id, pers_id_author, valid_from_timestamp)
  VALUES (nextval('public.data_set_relationships_history_id_seq'::regclass), new.id, 'OWNED'::text, new.samp_id, 'SAMPLE'::text, ( SELECT samples_all.perm_id
           FROM public.samples_all
...
CREATE RULE dataset_sample_update AS
  ...
  WHERE (((sample_relationships_history.main_samp_id)::bigint = (old.samp_id)::bigint) AND ((sample_relationships_history.data_id)::bigint = (old.id)::bigint) AND (sample_relationships_history.valid_until_timestamp IS NULL));
 INSERT INTO public.sample_relationships_history (id, main_samp_id, relation_type, data_id, entity_kind, entity_perm_id, pers_id_author, valid_from_timestamp)
  VALUES (nextval('public.sample_relationships_history_id_seq'::regclass), new.samp_id, 'OWNER'::text, new.id, 'DATA SET'::text, new.code, new.pers_id_modifier, new.modification_timestamp);
 UPDATE public.data_set_relationships_history SET valid_until_timestamp = new.modification_timestamp
  WHERE (((data_set_relationships_history.main_data_id)::bigint = (old.id)::bigint) AND ((data_set_relationships_history.samp_id)::bigint = (old.samp_id)::bigint) AND (data_set_relationships_history.valid_until_timestamp IS NULL));
 INSERT INTO public.data_set_relationships_history (id, main_data_id, relation_type, samp_id, entity_kind, entity_perm_id, pers_id_author, valid_from_timestamp)
...
CREATE RULE edms_b_expire_content_copy_history AS
    ON UPDATE TO public.external_data_management_systems DO  UPDATE public.data_set_copies_history SET valid_until_timestamp = now()
...
CREATE RULE experiment_project_insert AS
   ...
   WHERE (new.proj_id IS NOT NULL) DO ( INSERT INTO public.project_relationships_history (id, main_proj_id, relation_type, expe_id, entity_kind, entity_perm_id, pers_id_author, valid_from_timestamp)
  VALUES (nextval('public.project_relationships_history_id_seq'::regclass), new.proj_id, 'OWNER'::text, new.id, 'EXPERIMENT'::text, new.perm_id, new.pers_id_modifier, new.modification_timestamp);
 INSERT INTO public.experiment_relationships_history (id, main_expe_id, relation_type, proj_id, entity_kind, entity_perm_id, pers_id_author, valid_from_timestamp)
  VALUES (nextval('public.experiment_relationships_history_id_seq'::regclass), new.id, 'OWNED'::text, new.proj_id, 'PROJECT'::text, ( SELECT projects.perm_id
...
CREATE RULE experiment_project_update AS
  ...
  WHERE (((project_relationships_history.main_proj_id)::bigint = (old.proj_id)::bigint) AND ((project_relationships_history.expe_id)::bigint = (old.id)::bigint) AND (project_relationships_history.valid_until_timestamp IS NULL));
 INSERT INTO public.project_relationships_history (id, main_proj_id, relation_type, expe_id, entity_kind, entity_perm_id, pers_id_author, valid_from_timestamp)
  VALUES (nextval('public.project_relationships_history_id_seq'::regclass), new.proj_id, 'OWNER'::text, new.id, 'EXPERIMENT'::text, new.perm_id, new.pers_id_modifier, new.modification_timestamp);
 UPDATE public.experiment_relationships_history SET valid_until_timestamp = new.modification_timestamp
  WHERE (((experiment_relationships_history.main_expe_id)::bigint = (old.id)::bigint) AND ((experiment_relationships_history.proj_id)::bigint = (old.proj_id)::bigint) AND (experiment_relationships_history.valid_until_timestamp IS NULL));
 INSERT INTO public.experiment_relationships_history (id, main_expe_id, relation_type, proj_id, entity_kind, entity_perm_id, pers_id_author, valid_from_timestamp)
...
CREATE RULE experiment_properties_delete AS
   ...
   WHERE (((old.value IS NOT NULL) AND (decode(replace("substring"((old.value)::text, 1, 1), '\'::text, '\\'::text), 'escape'::text) <> '\xefbfbd'::bytea)) OR (old.cvte_id IS NOT NULL) OR (old.mate_prop_id IS NOT NULL) OR (old.samp_prop_id IS NOT NULL)) DO  INSERT INTO public.experiment_properties_history (id, expe_id, etpt_id, value, vocabulary_term, material, sample, pers_id_author, valid_from_timestamp, valid_until_timestamp)
...
CREATE RULE project_space_insert AS
   ...
   WHERE (new.space_id IS NOT NULL) DO  INSERT INTO public.project_relationships_history (id, main_proj_id, relation_type, space_id, entity_kind, entity_perm_id, pers_id_author, valid_from_timestamp)
  VALUES (nextval('public.project_relationships_history_id_seq'::regclass), new.id, 'OWNED'::text, new.space_id, 'SPACE'::text, ( SELECT spaces.code
           FROM public.spaces
...
CREATE RULE project_space_update AS
  ...
  WHERE (((project_relationships_history.main_proj_id)::bigint = (old.id)::bigint) AND ((project_relationships_history.space_id)::bigint = (old.space_id)::bigint) AND (project_relationships_history.valid_until_timestamp IS NULL));
 INSERT INTO public.project_relationships_history (id, main_proj_id, relation_type, space_id, entity_kind, entity_perm_id, pers_id_author, valid_from_timestamp)
  VALUES (nextval('public.project_relationships_history_id_seq'::regclass), new.id, 'OWNED'::text, new.space_id, 'SPACE'::text, ( SELECT spaces.code
           FROM public.spaces
          WHERE ((spaces.id)::bigint = (new.space_id)::bigint)), new.pers_id_modifier, new.modification_timestamp);
);
...
CREATE RULE sample_container_insert AS
   ...
   WHERE (new.samp_id_part_of IS NOT NULL) DO ( INSERT INTO public.sample_relationships_history (id, main_samp_id, relation_type, samp_id, entity_kind, entity_perm_id, pers_id_author, valid_from_timestamp)
  VALUES (nextval('public.sample_relationships_history_id_seq'::regclass), new.samp_id_part_of, 'CONTAINER'::text, new.id, 'SAMPLE'::text, new.perm_id, new.pers_id_modifier, new.modification_timestamp);
 INSERT INTO public.sample_relationships_history (id, main_samp_id, relation_type, samp_id, entity_kind, entity_perm_id, pers_id_author, valid_from_timestamp)
  VALUES (nextval('public.sample_relationships_history_id_seq'::regclass), new.id, 'CONTAINED'::text, new.samp_id_part_of, 'SAMPLE'::text, ( SELECT samples_all.perm_id
           FROM public.samples_all
...
CREATE RULE sample_experiment_insert AS
    ON INSERT TO public.samples_all
   WHERE (new.expe_id IS NOT NULL) DO ( INSERT INTO public.experiment_relationships_history (id, main_expe_id, relation_type, samp_id, entity_kind, entity_perm_id, pers_id_author, valid_from_timestamp)
  VALUES (nextval('public.experiment_relationships_history_id_seq'::regclass), new.expe_id, 'OWNER'::text, new.id, 'SAMPLE'::text, new.perm_id, new.pers_id_modifier, new.modification_timestamp);
 INSERT INTO public.sample_relationships_history (id, main_samp_id, relation_type, expe_id, entity_kind, entity_perm_id, pers_id_author, valid_from_timestamp)
  VALUES (nextval('public.sample_relationships_history_id_seq'::regclass), new.id, 'OWNED'::text, new.expe_id, 'EXPERIMENT'::text, ( SELECT experiments_all.perm_id
           FROM public.experiments_all
          WHERE ((experiments_all.id)::bigint = (new.expe_id)::bigint)), new.pers_id_modifier, new.modification_timestamp);
);
...
CREATE RULE sample_experiment_update AS
  ...
  WHERE (((experiment_relationships_history.main_expe_id)::bigint = (old.expe_id)::bigint) AND ((experiment_relationships_history.samp_id)::bigint = (old.id)::bigint) AND (experiment_relationships_history.valid_until_timestamp IS NULL));
 INSERT INTO public.experiment_relationships_history (id, main_expe_id, relation_type, samp_id, entity_kind, entity_perm_id, pers_id_author, valid_from_timestamp)
  VALUES (nextval('public.experiment_relationships_history_id_seq'::regclass), new.expe_id, 'OWNER'::text, new.id, 'SAMPLE'::text, new.perm_id, new.pers_id_modifier, new.modification_timestamp);
 UPDATE public.sample_relationships_history SET valid_until_timestamp = new.modification_timestamp
  WHERE (((sample_relationships_history.main_samp_id)::bigint = (old.id)::bigint) AND ((sample_relationships_history.expe_id)::bigint = (old.expe_id)::bigint) AND (sample_relationships_history.valid_until_timestamp IS NULL));
 INSERT INTO public.sample_relationships_history (id, main_samp_id, relation_type, expe_id, entity_kind, entity_perm_id, pers_id_author, valid_from_timestamp)
...
CREATE RULE sample_parent_annotations_update AS
    ON UPDATE TO public.sample_relationships_all
   WHERE ((old.del_id IS NULL) AND (new.del_id IS NULL) AND ((old.sample_id_child)::bigint = (new.sample_id_child)::bigint) AND ((old.sample_id_parent)::bigint = (new.sample_id_parent)::bigint) AND (old.parent_annotations <> new.parent_annotations)) DO  INSERT INTO public.sample_relationships_history (id, main_samp_id, relation_type, samp_id, entity_kind, entity_perm_id, annotations, pers_id_author, valid_from_timestamp)
  VALUES (nextval('public.sample_relationships_history_id_seq'::regclass), new.sample_id_parent, 'PARENT'::text, new.sample_id_child, 'SAMPLE'::text, ( SELECT samples_all.perm_id
           FROM public.samples_all
...
CREATE RULE sample_parent_child_insert AS
   ...
   WHERE (new.del_id IS NULL) DO ( INSERT INTO public.sample_relationships_history (id, main_samp_id, relation_type, samp_id, entity_kind, entity_perm_id, annotations, pers_id_author, valid_from_timestamp)
  VALUES (nextval('public.sample_relationships_history_id_seq'::regclass), new.sample_id_parent, 'PARENT'::text, new.sample_id_child, 'SAMPLE'::text, ( SELECT samples_all.perm_id
           FROM public.samples_all
 ...
 INSERT INTO public.sample_relationships_history (id, main_samp_id, relation_type, samp_id, entity_kind, entity_perm_id, annotations, pers_id_author, valid_from_timestamp)
  VALUES (nextval('public.sample_relationships_history_id_seq'::regclass), new.sample_id_child, 'CHILD'::text, new.sample_id_parent, 'SAMPLE'::text, ( SELECT samples_all.perm_id
           FROM public.samples_all
...
CREATE RULE sample_parent_child_revert_update AS
    ON UPDATE TO public.sample_relationships_all
   WHERE ((new.del_id IS NULL) AND (old.del_id IS NOT NULL)) DO ( INSERT INTO public.sample_relationships_history (id, main_samp_id, relation_type, samp_id, entity_kind, entity_perm_id, annotations, pers_id_author, valid_from_timestamp)
  VALUES (nextval('public.sample_relationships_history_id_seq'::regclass), new.sample_id_parent, 'PARENT'::text, new.sample_id_child, 'SAMPLE'::text, ( SELECT samples_all.perm_id
           FROM public.samples_all
          WHERE ((samples_all.id)::bigint = (new.sample_id_child)::bigint)), new.parent_annotations, new.pers_id_author, new.modification_timestamp);
 INSERT INTO public.sample_relationships_history (id, main_samp_id, relation_type, samp_id, entity_kind, entity_perm_id, annotations, pers_id_author, valid_from_timestamp)
...
CREATE RULE sample_project_insert AS
    ON INSERT TO public.samples_all
   WHERE ((new.expe_id IS NULL) AND (new.proj_id IS NOT NULL)) DO ( INSERT INTO public.project_relationships_history (id, main_proj_id, relation_type, samp_id, entity_kind, entity_perm_id, pers_id_author, valid_from_timestamp)
  VALUES (nextval('public.project_relationships_history_id_seq'::regclass), new.proj_id, 'OWNER'::text, new.id, 'SAMPLE'::text, new.perm_id, new.pers_id_modifier, new.modification_timestamp);
 INSERT INTO public.sample_relationships_history (id, main_samp_id, relation_type, proj_id, entity_kind, entity_perm_id, pers_id_author, valid_from_timestamp)
  VALUES (nextval('public.sample_relationships_history_id_seq'::regclass), new.id, 'OWNED'::text, new.proj_id, 'PROJECT'::text, ( SELECT projects.perm_id
           FROM public.projects
          WHERE ((projects.id)::bigint = (new.proj_id)::bigint)), new.pers_id_modifier, new.modification_timestamp);
);
CREATE RULE sample_project_remove_update AS
    ON UPDATE TO public.samples_all
   WHERE ((old.proj_id IS NOT NULL) AND ((new.proj_id IS NULL) OR ((old.expe_id IS NULL) AND (new.expe_id IS NOT NULL)))) DO ( UPDATE public.project_relationships_history SET valid_until_timestamp = new.modification_timestamp
...
CREATE RULE sample_project_update AS
    ON UPDATE TO public.samples_all
   WHERE ((((old.proj_id)::bigint <> (new.proj_id)::bigint) OR (old.proj_id IS NULL) OR (old.expe_id IS NOT NULL)) AND (new.proj_id IS NOT NULL) AND (new.expe_id IS NULL)) DO ( UPDATE public.project_relationships_history SET valid_until_timestamp = new.modification_timestamp
  WHERE (((project_relationships_history.main_proj_id)::bigint = (old.proj_id)::bigint) AND ((project_relationships_history.samp_id)::bigint = (old.id)::bigint) AND (project_relationships_history.valid_until_timestamp IS NULL));
 INSERT INTO public.project_relationships_history (id, main_proj_id, relation_type, samp_id, entity_kind, entity_perm_id, pers_id_author, valid_from_timestamp)
  VALUES (nextval('public.project_relationships_history_id_seq'::regclass), new.proj_id, 'OWNER'::text, new.id, 'SAMPLE'::text, new.perm_id, new.pers_id_modifier, new.modification_timestamp);
 UPDATE public.sample_relationships_history SET valid_until_timestamp = new.modification_timestamp
  WHERE (((sample_relationships_history.main_samp_id)::bigint = (old.id)::bigint) AND ((sample_relationships_history.proj_id)::bigint = (old.proj_id)::bigint) AND (sample_relationships_history.valid_until_timestamp IS NULL));
 INSERT INTO public.sample_relationships_history (id, main_samp_id, relation_type, proj_id, entity_kind, entity_perm_id, pers_id_author, valid_from_timestamp)
...
CREATE RULE sample_properties_delete AS
   ...
   WHERE ((((old.value IS NOT NULL) AND (decode(replace("substring"((old.value)::text, 1, 1), '\'::text, '\\'::text), 'escape'::text) <> '\xefbfbd'::bytea)) OR (old.cvte_id IS NOT NULL) OR (old.mate_prop_id IS NOT NULL) OR (old.samp_prop_id IS NOT NULL)) AND (( SELECT samples_all.del_id
           FROM public.samples_all
          WHERE ((samples_all.id)::bigint = (old.samp_id)::bigint)) IS NULL)) DO  INSERT INTO public.sample_properties_history (id, samp_id, stpt_id, value, vocabulary_term, material, sample, pers_id_author, valid_from_timestamp, valid_until_timestamp)
...
CREATE RULE sample_properties_update AS
   ...
   WHERE (((old.value IS NOT NULL) AND (decode(replace("substring"((old.value)::text, 1, 1), '\'::text, '\\'::text), 'escape'::text) <> '\xefbfbd'::bytea) AND ((old.value)::text <> (new.value)::text)) OR ((old.cvte_id IS NOT NULL) AND ((old.cvte_id)::bigint <> (new.cvte_id)::bigint)) OR ((old.mate_prop_id IS NOT NULL) AND ((old.mate_prop_id)::bigint <> (new.mate_prop_id)::bigint)) OR ((old.samp_prop_id IS NOT NULL) AND ((old.samp_prop_id)::bigint <> (new.samp_prop_id)::bigint))) DO  INSERT INTO public.sample_properties_history (id, samp_id, stpt_id, value, vocabulary_term, material, sample, pers_id_author, valid_from_timestamp, valid_until_timestamp)
  VALUES (nextval('public.sample_property_id_seq'::regclass), old.samp_id, old.stpt_id, old.value, ( SELECT ((((t.code)::text || ' ['::text) || (v.code)::text) || ']'::text)
           FROM (public.controlled_vocabulary_terms t
             JOIN public.controlled_vocabularies v ON (((t.covo_id)::bigint = (v.id)::bigint)))
          WHERE ((t.id)::bigint = (old.cvte_id)::bigint)), ( SELECT ((((m.code)::text || ' ['::text) || (mt.code)::text) || ']'::text)
...
CREATE RULE sample_relationships_insert AS
    ON INSERT TO public.sample_relationships DO INSTEAD  INSERT INTO public.sample_relationships_all (id, sample_id_parent, parent_frozen, relationship_id, sample_id_child, child_frozen, pers_id_author, registration_timestamp, modification_timestamp, child_annotations, parent_annotations)
...
CREATE RULE sample_shared_insert AS
    ON INSERT TO public.samples_all
   WHERE (new.space_id IS NULL) DO  INSERT INTO public.sample_relationships_history (id, main_samp_id, relation_type, expe_id, pers_id_author, valid_from_timestamp)
  VALUES (nextval('public.sample_relationships_history_id_seq'::regclass), new.id, 'OWNED'::text, new.expe_id, new.pers_id_modifier, new.modification_timestamp);
CREATE RULE sample_shared_remove_update AS
    ON UPDATE TO public.samples_all
   WHERE ((old.space_id IS NULL) AND (new.space_id IS NOT NULL)) DO  UPDATE public.sample_relationships_history SET valid_until_timestamp = new.modification_timestamp
...
CREATE RULE sample_shared_update AS
    ON UPDATE TO public.samples_all
   WHERE ((old.space_id IS NOT NULL) AND (new.space_id IS NULL)) DO ( UPDATE public.sample_relationships_history SET valid_until_timestamp = new.modification_timestamp
 ...
 INSERT INTO public.sample_relationships_history (id, main_samp_id, relation_type, space_id, entity_perm_id, pers_id_author, valid_from_timestamp)
  VALUES (nextval('public.sample_relationships_history_id_seq'::regclass), new.id, 'OWNED'::text, new.space_id, NULL::text, new.pers_id_modifier, new.modification_timestamp);
);
CREATE RULE sample_space_insert AS
    ON INSERT TO public.samples_all
   WHERE ((new.expe_id IS NULL) AND (new.space_id IS NOT NULL) AND (new.proj_id IS NULL)) DO  INSERT INTO public.sample_relationships_history (id, main_samp_id, relation_type, space_id, entity_kind, entity_perm_id, pers_id_author, valid_from_timestamp)
  VALUES (nextval('public.sample_relationships_history_id_seq'::regclass), new.id, 'OWNED'::text, new.space_id, 'SPACE'::text, ( SELECT spaces.code
           FROM public.spaces
          WHERE ((spaces.id)::bigint = (new.space_id)::bigint)), new.pers_id_modifier, new.modification_timestamp);
...
CREATE RULE sample_space_update AS
  ...
  WHERE (((sample_relationships_history.main_samp_id)::bigint = (old.id)::bigint) AND ((sample_relationships_history.space_id)::bigint = (old.space_id)::bigint) AND (sample_relationships_history.valid_until_timestamp IS NULL));
 INSERT INTO public.sample_relationships_history (id, main_samp_id, relation_type, space_id, entity_kind, entity_perm_id, pers_id_author, valid_from_timestamp)
  VALUES (nextval('public.sample_relationships_history_id_seq'::regclass), new.id, 'OWNED'::text, new.space_id, 'SPACE'::text, ( SELECT spaces.code
           FROM public.spaces
          WHERE ((spaces.id)::bigint = (new.space_id)::bigint)), new.pers_id_modifier, new.modification_timestamp);
);
...
CREATE TRIGGER data_all_tsvector_document BEFORE INSERT OR UPDATE ON public.data_all FOR EACH ROW EXECUTE PROCEDURE public.data_all_tsvector_document_trigger();
...
CREATE TRIGGER data_set_properties_tsvector_document BEFORE INSERT OR UPDATE ON public.data_set_properties FOR EACH ROW EXECUTE PROCEDURE public.properties_tsvector_document_trigger();
...
CREATE TRIGGER experiment_properties_tsvector_document BEFORE INSERT OR UPDATE ON public.experiment_properties FOR EACH ROW EXECUTE PROCEDURE public.properties_tsvector_document_trigger();
...
CREATE TRIGGER experiments_all_in_project_tsvector_document AFTER UPDATE ON public.projects FOR EACH ROW EXECUTE PROCEDURE public.experiments_all_in_project_tsvector_document_trigger();
CREATE TRIGGER experiments_all_tsvector_document BEFORE INSERT OR UPDATE ON public.experiments_all FOR EACH ROW EXECUTE PROCEDURE public.experiments_all_tsvector_document_trigger();
...
CREATE TRIGGER material_properties_tsvector_document BEFORE INSERT OR UPDATE ON public.material_properties FOR EACH ROW EXECUTE PROCEDURE public.properties_tsvector_document_trigger();
...
CREATE TRIGGER materials_tsvector_document BEFORE INSERT OR UPDATE ON public.materials FOR EACH ROW EXECUTE PROCEDURE public.materials_tsvector_document_trigger();
...
CREATE TRIGGER sample_properties_tsvector_document BEFORE INSERT OR UPDATE ON public.sample_properties FOR EACH ROW EXECUTE PROCEDURE public.properties_tsvector_document_trigger();
...
CREATE TRIGGER samples_all_tsvector_document BEFORE INSERT OR UPDATE ON public.samples_all FOR EACH ROW EXECUTE PROCEDURE public.samples_all_tsvector_document_trigger();
...
ALTER TABLE ONLY public.data_set_properties
    ADD CONSTRAINT data_set_properties_unique_fk FOREIGN KEY (dstpt_id, is_unique) REFERENCES public.data_set_type_property_types(id, is_unique);
ALTER TABLE ONLY public.data_set_relationships_all
...
ALTER TABLE ONLY public.events_search
    ADD CONSTRAINT events_search_exac_id_fk FOREIGN KEY (exac_id) REFERENCES public.attachment_contents(id);
ALTER TABLE ONLY public.events_search
    ADD CONSTRAINT events_search_pers_id_registerer_fk FOREIGN KEY (pers_id_registerer) REFERENCES public.persons(id) DEFERRABLE INITIALLY DEFERRED;
ALTER TABLE ONLY public.events
...
ALTER TABLE ONLY public.experiment_properties
    ADD CONSTRAINT experiment_properties_unique_fk FOREIGN KEY (etpt_id, is_unique) REFERENCES public.experiment_type_property_types(id, is_unique);
ALTER TABLE ONLY public.experiment_properties
...
ALTER TABLE ONLY public.material_properties
    ADD CONSTRAINT material_properties_unique_fk FOREIGN KEY (mtpt_id, is_unique) REFERENCES public.material_type_property_types(id, is_unique);
ALTER TABLE ONLY public.material_types
...
ALTER TABLE ONLY public.persons
    ADD CONSTRAINT pers_space_fk FOREIGN KEY (space_id) REFERENCES public.spaces(id) ON DELETE SET NULL;
ALTER TABLE ONLY public.post_registration_dataset_queue
...
ALTER TABLE ONLY public.project_relationships_history
    ADD CONSTRAINT prrelh_samp_fk FOREIGN KEY (samp_id) REFERENCES public.samples_all(id) ON DELETE SET NULL;
ALTER TABLE ONLY public.project_relationships_history
...
ALTER TABLE ONLY public.property_types
    ADD CONSTRAINT prty_saty_fk FOREIGN KEY (saty_prop_id) REFERENCES public.sample_types(id) ON DELETE CASCADE;
ALTER TABLE ONLY public.queries
...
ALTER TABLE ONLY public.sample_properties
    ADD CONSTRAINT sample_properties_unique_fk FOREIGN KEY (stpt_id, is_unique) REFERENCES public.sample_type_property_types(id, is_unique);
ALTER TABLE ONLY public.sample_relationships_all
