var ExperimentDataGridUtil = new (function () {
  this.getExperimentDataGrid = function (
    typeCode,
    entities,
    rowClick,
    heightPercentage
    ...
    var propertyCodes =
      profile.getAllPropertiCodesForExperimentTypeCode(typeCode);
    var propertyCodesDisplayNames =
      profile.getPropertiesDisplayNamesForExperimentTypeCode(
        typeCode,
        propertyCodes
      );
    ...
    if ($.inArray("$NAME", propertyCodes) !== -1) {
      columns.push({
        ...
        render: function (data) {
          return FormUtil.getFormLink(
            data[profile.propertyReplacingCode],
            "Experiment",
            data.identifier
          );
        },
        ...
        var getVocabularyColumn = function (propertyType) {
          return function () {
            return {
              ...
              sortable: true,
              render: function (data) {
                return FormUtil.getVocabularyLabelForTermCode(
                  propertyType,
                  data[propertyType.code]
                );
              },
              filter: function (data, filter) {
                var value = FormUtil.getVocabularyLabelForTermCode(
                  propertyType,
                  data[propertyType.code]
                );
              ...
              sort: function (data1, data2, asc) {
                var value1 = FormUtil.getVocabularyLabelForTermCode(
                  propertyType,
                  data1[propertyType.code]
                );
                if (!value1) {
                  value1 = "";
                }
                var value2 = FormUtil.getVocabularyLabelForTermCode(
                  propertyType,
                  data2[propertyType.code]
                );
                if (!value2) {
                  value2 = "";
                }
    ...
    var getDataList = function (callback) {
      ...
      for (var sIdx = 0; sIdx < entities.length; sIdx++) {
        ...
        var model = {
          ...
          registrationDate: Util.getFormatedDate(
            new Date(entity.registrationDetails.registrationDate)
          ),
          modifier: entity.registrationDetails.modifierUserId,
          modificationDate: Util.getFormatedDate(
            new Date(entity.registrationDetails.modificationDate)
          ),
        };
    ...
    var dataGridController = new DataGridController(
      null,
      columns,
      [],
      null,
