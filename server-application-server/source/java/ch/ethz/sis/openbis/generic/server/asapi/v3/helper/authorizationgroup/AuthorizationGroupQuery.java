...
{
    @Select(sql = "select id, code as identifier from authorization_groups where code = any(?{1})", parameterBindings = {
            StringArrayMapper.class }, fetchSize = FETCH_SIZE)
    public List<TechIdStringIdentifierRecord> listAuthorizationGroupTechIdsByPermIds(String[] permIds);
}
