 ...
 */
@Component
public class DataSetLinkedDataTranslator extends ObjectToOneRelationTranslator<LinkedData, LinkedDataFetchOptions> implements
        IDataSetLinkedDataTranslator
{

    @Autowired
    private ILinkedDataTranslator linkedDataTranslator;

    @Override
    protected List<ObjectRelationRecord> loadRecords(LongOpenHashSet objectIds)
    {
        DataSetQuery query = QueryTool.getManagedQuery(DataSetQuery.class);
        return query.getLinkedDataIds(objectIds);
    }

    @Override
    protected Map<Long, LinkedData> translateRelated(TranslationContext context, Collection<Long> relatedIds,
            LinkedDataFetchOptions relatedFetchOptions)
    {
        return linkedDataTranslator.translate(context, relatedIds, relatedFetchOptions);
    }
