...
public class CreationIdCache
{
    ...
    private Map<CreationId, IObjectId> creationIdToRealIdMap = new HashMap<>();

    private Map<IObjectId, CreationId> realIdToCreationIdMap = new HashMap<>();

    public void putIds(CreationId creationId, IObjectId realId)
    {
        creationIdToRealIdMap.put(creationId, realId);
        realIdToCreationIdMap.put(realId, creationId);
    }

    public IObjectId getRealId(CreationId creationId)
    {
        return creationIdToRealIdMap.get(creationId);
    }

    public CreationId getCreationId(IObjectId realId)
    {
        return realIdToCreationIdMap.get(realId);
    }
