...
import ch.ethz.sis.openbis.generic.server.asapi.v3.helper.semanticannotation.ListSemanticAnnotationByPermId;
 ...
 */
@Component
public class MapSemanticAnnotationByIdExecutor extends AbstractMapObjectByIdExecutor<ISemanticAnnotationId, SemanticAnnotationPE>
        implements IMapSemanticAnnotationByIdExecutor
{
    ...
    @Override
    protected void addListers(IOperationContext context, List<IListObjectById<? extends ISemanticAnnotationId, SemanticAnnotationPE>> listers)
    {
        listers.add(new ListSemanticAnnotationByPermId(semanticAnnotationDAO));
    }

    @Autowired
    private void setDAOFactory(IDAOFactory daoFactory)
    {
        semanticAnnotationDAO = daoFactory.getSemanticAnnotationDAO();
    }
