...
public class ProjectIdentifierExistingSpacePredicateSystemTest extends CommonPredicateSystemTest<ProjectIdentifier>
{
    ...
    @Override
    protected void evaluateObjects(ProjectAuthorizationUser user, List<ProjectIdentifier> objects, Object param)
    {
        getBean(ProjectPredicateTestService.class).testProjectIdentifierExistingSpacePredicate(user.getSessionProvider(), objects.get(0));
    }

    @Override
    protected CommonPredicateSystemTestAssertions<ProjectIdentifier> getAssertions()
    {
        return new CommonPredicateSystemTestAssertionsDelegate<ProjectIdentifier>(super.getAssertions())
            {
                @Override
